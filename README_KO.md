...
**`wrk`**를 사용하여 테스트했습니다 (125개 연결, 4스레드, 30초).
Hon은 표준 라이브러리보다 더 높은 처리량과 안정성을 보여줍니다.

| 지표 | 표준 서버 (`net/http`) | **Hon** | 개선 효과 |
| :--- | :--- | :--- | :--- |
| **초당 요청 (RPS)** | 197,387 | **214,640** | **~8.7% 빠름** |
| **처리량 (Throughput)** | 86.22MB/s | **97.64MB/s** | **대역폭 증가** |
| **소켓 에러** | 86 (connect) | **0** | **완전 무결** |

> **결과**: Hon은 높은 부하 상황에서도 표준 서버보다 더 많은 요청을 처리하며, 연결 에러(Socket Errors)가 전혀 발생하지 않았습니다. 단순 호환성을 넘어 성능 업그레이드를 제공합니다.

### 2. WebSocket 동시성 (자원 효율)

단일 머신(macOS)에서 **15,000개 동시 WebSocket 접속** 테스트 결과:
(공정한 비교를 위해 두 서버 모두 `reuseport`를 사용했습니다.)

| 지표 | 표준 서버 (`net/http`) | **Hon (`Netpoll`)** | 개선 효과 |
| :--- | :--- | :--- | :--- |
| **연결 수** | 15,000개 (안정적) | **15,000개 (안정적)** | **동등한 안정성** |
| **고루틴 개수** | **15,005 개** | **6 개** | **~99.9% 절감** |
| **아키텍처** | 연결당 고루틴 할당 | **이벤트 기반 Reactor** | **Non-blocking I/O** |

> **핵심 요약**: 두 서버 모두 15,000개의 연결을 안정적으로 유지했습니다. 하지만 표준 서버는 이를 위해 **15,005개의 고루틴**을 사용한 반면, **Hon**은 단 **6개의 고루틴**만으로 동일한 부하를 처리했습니다. 이러한 리소스 효율성은 수십만 연결 이상의 대규모 서비스 확장을 가능하게 합니다.
